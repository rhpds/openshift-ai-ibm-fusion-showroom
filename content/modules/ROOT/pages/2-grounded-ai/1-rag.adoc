...
+
image::2-grounded-ai/llama-stack-upgrade-1.png[llama-stack-upgrade-1]

. Switch to the Form view and simply check the "enabled" checkbox under the RAG configuration to activate these capabilities.
+
image::2-grounded-ai/llama-stack-upgrade-2.png[llama-stack-upgrade-2]
+
..and click `Upgrade`.

. Before we jump into trying out RAG, let's get a better understanding of how *embeddings* work.
+
...
+
When you finish, come back so we can continue with *xref:2-grounded-ai/2-docling.adoc[Docling]* to learn how to work with more complex data 🐣
